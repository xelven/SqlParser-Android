/**
 * Copyright 2020 Allen Chan xelven@gmail.com
 *
...
public final class SQLParser {
	...
	private static final String TRIGGER_STATMENT_END = "END";
				...
				for (; i < text.length(); i++) {
					...
					if (inStringConstant == false && inLineComment == false && inBlockComment == false) {
						if (c == '\'') { // String-Konstante ??
							...
							if (c == statementEnd) {
								if (plsql) {
									if (temp.toString().toUpperCase().endsWith(TRIGGER_STATMENT_END)) {
										endOfStatement = true;
									} else {
										temp.append(c);
									}
	...
	private boolean testForPLSQL(String text, int startPos) {
		if (useScriptDetecting == false) {
			...
			for (int i = 0; i < plsqlKeyWords.length; i++) {
				if (test.toUpperCase().startsWith(plsqlKeyWords[i])) {
					testResult = true;
					break;
				}
			} // for (int i=0; i < plsqlKeyWords.length; i++)
	...
	public static String formatSQL(String sql) {
		...
		final ParserStringReplacer sr = new ParserStringReplacer(sql.trim());
		sr.replace("\n", " ");
		sr.replace("\r", "");
		sr.replace("\t", " ");
	 ...
	 * @param sqlcode Code des prepared statement
	...
	public static void setupPreparedStatementParams(SQLStatement ps) {
		...
		if (hasNamedParameter) {
			...
			ParserStringReplacer sr = new ParserStringReplacer(sql);
			for (SQLParameter p : ps.getParams()) {
				if (p.isNamedParam()) {
