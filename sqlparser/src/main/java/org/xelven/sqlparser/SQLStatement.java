...
public class SQLStatement implements Serializable {
	...
	private List<SQLParameter>   paramList                  = new Vector<SQLParameter>();
	...
	public SQLParameter getParam(String name) {
		SQLParameter psParam = null;
		for (int i = 0; i < paramList.size(); i++) {
			psParam = (SQLParameter) paramList.get(i);
	...
	public SQLParameter getParam(int paramIndex) {
		SQLParameter psParam = null;
		for (int i = 0; i < paramList.size(); i++) {
	...
	public boolean hasOutputParams() {
		for (SQLParameter psParam : paramList) {
	...
	public List<SQLParameter> getOutputParams() {
		final ArrayList<SQLParameter> list = new ArrayList<SQLParameter>();
		for (SQLParameter psParam : paramList) {
			if (psParam.isOutParam()) {
				list.add(psParam);
			}
